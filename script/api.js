...
( async function () {
  ...
  components.forEach((comp) => {
    ...
    children.forEach((el) => {
      ...
      if (el.type === 'fence') {
        writeAutoGeneratedPage(compTag, el.name, renderCode(el));
      } else if (el.p) {
        let content = el.p.join('\n\n') +'\n\n'+ renderProperties(el.children);
        writeAutoGeneratedPage(compTag, el.name, content);
      } else {
...
function renderCode({info, content}) {
  let str = "```" + info + '\n' + content + '\n' + "```"
  return str
